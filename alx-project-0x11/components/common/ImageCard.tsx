...
const ImageCard: React.FC<GeneratedImageProps> = ({imageUrl,prompt,width,action}) => {
    ...
    return(
        <div onClick={() => action(imageUrl)} className="p-6 mt-6 hover:cursor-pointer border">
            <img src={imageUrl} alt={prompt} className="w-full max-w-md rounded-lg shadow-lg  hover:scale-105" />
